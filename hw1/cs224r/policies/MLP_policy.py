...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        return ptu.to_numpy(self.forward(observation).rsample())
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        if self.discrete:
            return distributions.Categorical(self.logits_na(obs))
        else:
            return distributions.MultivariateNormal(self.mean_net(obs), torch.diag(torch.exp(self.logstd)))
    ...
    def update(self, observations, actions):
        ...
        if self.discrete:
            ...
            loss = cross_entropy_loss(self.forward((observations)).rsample(), act)
        else:
            #mse_loss = nn.MSELoss()
            #loss = mse_loss(self.forward((observations)).rsample(), act)
            loss = -self.forward((observations)).log_prob(act).mean()
